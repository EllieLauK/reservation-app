        ...
        return (
          <div key={label} className="flex items-center gap-2">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold shrink-0
              ${done ? 'bg-purple-700 text-white' : active ? 'bg-purple-700 text-white ring-4 ring-purple-100' : 'bg-stone-100 text-stone-400'}`}>
              {done ? '✓' : num}
            </div>
            <span className={`text-sm hidden sm:block ${active ? 'text-stone-900 font-medium' : 'text-stone-400'}`}>{label}</span>
            {i < labels.length - 1 && <div className={`h-px w-8 sm:w-12 ${done ? 'bg-purple-400' : 'bg-stone-200'}`} />}
          </div>
        )
      })}
    </div>
      ...
      <div className="flex gap-2 flex-wrap mb-6">
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors
            ${filter === 'all' ? 'bg-purple-700 text-white' : 'bg-purple-50 text-purple-800 hover:bg-purple-100'}`}
        >
          {dict.all}
        </button>
        {categoryKeys.map(cat => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors
              ${filter === cat ? 'bg-purple-700 text-white' : 'bg-purple-50 text-purple-800 hover:bg-purple-100'}`}
          >
            {categories[cat]}
          </button>
        ))}
          ...
          <button
            ...
            className={`text-left p-4 rounded-xl border-2 transition-all
              ${selected === service.id
                ...
                : 'border-stone-100 bg-white hover:border-purple-200 hover:shadow-sm'}`}
          >
            <div className="flex justify-between items-start mb-1">
              <span className="font-semibold text-stone-900">{service.name}</span>
              <span className="font-bold text-purple-700 text-sm ml-2 shrink-0">{formatPrice(service.price)}</span>
            </div>
        ...
        {staff.map(member => (
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-purple-700 mb-2">{member.role}</p>
              <p className="text-xs text-stone-500">{member.bio}</p>
            </div>
            <div className="flex flex-wrap gap-1 justify-center">
              {member.specialties.map(s => (
                <span key={s} className="text-xs bg-white border border-purple-100 text-purple-800 px-2 py-0.5 rounded-full">{s}</span>
              ))}
            </div>
          </button>
        ))}
  ...
  return (
    <div className="bg-white rounded-xl border border-stone-100 p-4 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <button onClick={() => setMonth(new Date(year, monthIndex - 1, 1))} className="p-1.5 rounded-lg hover:bg-purple-50 text-stone-500 hover:text-purple-800 transition-colors">‹</button>
          ...
          return (
            <button
              ...
              className={`h-9 w-9 rounded-full text-sm font-medium mx-auto flex items-center justify-center transition-colors
                ...
                ${!isSelected && available ? 'text-stone-800 hover:bg-purple-50' : ''}
                ...
                <button
                  ...
                  className={`py-2 rounded-lg text-sm font-medium border transition-colors
                    ${selectedTime === slot
                      ...
                      : 'bg-white border-stone-200 text-stone-700 hover:border-purple-300 hover:text-purple-800'}`}
                >
                  {slot}
                </button>
              ))}
    ...
    <div>
      ...
      <div className="bg-purple-50 rounded-xl p-4 mb-6 border border-purple-100">
        ...
        <div className="grid grid-cols-2 gap-y-1.5 text-sm">
          ...
          <span className="text-stone-500">{dict.price}</span><span className="font-medium text-purple-700">{service ? formatPrice(service.price) : ''}</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        <div>
          ...
          <input type="text" value={data.name} onChange={e => onNameChange(e.target.value)} placeholder="Jane Smith"
            className="w-full border border-stone-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-300 focus:border-purple-400" />
        </div>
        <div>
        ...
        <div className="sm:col-span-2">
          ...
          <textarea value={data.notes} onChange={e => onNotesChange(e.target.value)} placeholder={dict.notesPlaceholder} rows={3}
            className="w-full border border-stone-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-300 focus:border-purple-400 resize-none" />
        </div>
      </div>
      ...
      <button
        ...
        className="w-full bg-purple-700 text-white py-3 rounded-xl font-semibold hover:bg-purple-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-purple-100"
      >
...
function SuccessStep({ data, dict, locale }: { data: BookingData; dict: Dictionary['booking']['success']; locale: string }) {
  ...
  return (
    <div className="text-center py-8">
      <div className="w-20 h-20 bg-purple-100 rounded-full flex items-center justify-center mx-auto mb-6 text-4xl">✨</div>
      ...
      <div className="bg-purple-50 rounded-xl p-5 text-left max-w-sm mx-auto border border-purple-100 mb-8">
        ...
        <div className="space-y-1.5 text-sm">
          ...
          <div className="flex justify-between"><span className="text-stone-500">{dict.total}</span><span className="font-semibold text-purple-700">{service ? formatPrice(service.price) : ''}</span></div>
        </div>
      </div>
      <a href={`/${locale}`} className="inline-block text-purple-700 font-medium hover:text-purple-800 text-sm">{dict.back}</a>
    </div>
  )
}
      ...
      {step < 4 && step < 5 && (
        <div className="flex justify-between mt-4">
          ...
          <button onClick={() => setStep(s => s + 1)} disabled={!canNext[step]}
            className="px-6 py-2.5 rounded-xl text-sm font-medium bg-purple-700 text-white hover:bg-purple-800 transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
            {dict.continue}
          </button>
        </div>
      )}
